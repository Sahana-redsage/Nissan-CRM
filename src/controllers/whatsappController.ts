...
export const whatsappController = {
    async sendInsightSummary(req: AuthRequest, res: Response) {
        try {
            ...
            const baseMessage = await llmService.generateWhatsappSummary(vehicleData, insight.insightsJson as any);

            // Append UI Link
            const uiLink = `${config.frontendUrl}/insights/${insight.id}`;
            const personalizedMessage = baseMessage.replace('Customer', customer.customerName || 'Customer') +
                `\n\nView full details here: ${uiLink}`;
